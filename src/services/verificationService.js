    ...
    try {
        ...
        if (!guild) {
            throw createError(
                ...
                "Không tìm thấy máy chủ trong bộ nhớ của bot.",
                { guildId }
            );
        }
        ...
        try {
            ...
            throw createError(
                ...
                "Người dùng không có trong máy chủ này.",
                { userId, guildId }
            );
        }
        ...
        if (!guildConfig.verification?.enabled) {
            throw createError(
                ...
                ErrorTypes.CONFIGURATION,
                "Hệ thống xác minh chưa được bật trên máy chủ này.",
                { guildId }
            );
        }
        ...
        if (!canAssignRole) {
            throw createError(
                ...
                "Tôi không thể gán vai trò đã xác minh. Vui lòng kiểm tra quyền **Manage Roles** và thứ bậc vai trò của tôi.",
        ...
        const typedError = ensureTypedServiceError(error, {
            ...
            userMessage: 'Xác minh thất bại. Vui lòng thử lại sau ít phút nữa.',
...
export async function autoVerifyOnJoin(client, guild, member, verificationConfig) {
    try {
        ...
        const typedError = ensureTypedServiceError(error, {
            ...
            userMessage: 'Xác minh tự động thất bại. Vui lòng xác minh thủ công.',
    ...
    try {
        ...
        if (!guildConfig.verification?.enabled) {
            throw createError(
                "Verification system disabled",
                ErrorTypes.CONFIGURATION,
                "Hệ thống xác minh chưa được bật.",
                { guildId }
            );
        }
        ...
        if (!verifiedRole) {
            throw createError(
                ...
                ErrorTypes.CONFIGURATION,
                "Vai trò đã xác minh không còn tồn tại.",
        ...
        if (!canAssignRole) {
            throw createError(
                ...
                ErrorTypes.PERMISSION,
                "Tôi không thể gỡ vai trò đã xác minh ngay lúc này. Vui lòng kiểm tra quyền **Manage Roles** và thứ bậc vai trò của tôi.",
                { guildId, roleId: verifiedRole.id }
            );
        }
        ...
        const typedError = ensureTypedServiceError(error, {
            ...
            userMessage: 'Không thể gỡ xác minh. Vui lòng thử lại sau ít phút nữa.',
...
export async function validateVerificationSetup(guild, verificationConfig) {
    ...
    if (!botMember) {
        throw createError(
            ...
            ErrorTypes.CONFIGURATION,
            "Tôi không thể kiểm tra quyền của mình trên máy chủ. Vui lòng thử lại.",
            { guildId: guild.id }
        );
    }
    ...
    if (!verifiedRole) {
        throw createError(
            "Verified role not found",
            ErrorTypes.CONFIGURATION,
            "Vai trò đã xác minh đã bị xóa. Vui lòng chạy lại `/verification setup`.",
    ...
    if (verificationConfig.channelId) {
        ...
        if (missingPerms.length > 0) {
            throw createError(
                ...
                ErrorTypes.PERMISSION,
                `Tôi đang thiếu quyền trong kênh xác minh: ${missingPerms.join(', ')}`,
...
export async function checkVerificationCooldown(userId, guildId, cooldownMs = defaultCooldownMs) {
    ...
    if (lastVerified && Date.now() - lastVerified < cooldownMs) {
        ...
        throw createError(
            ...
            `Vui lòng đợi ${Math.ceil(remaining / 1000)} giây trước khi xác minh lại.`,
...
export async function trackVerificationAttempt(
    ...
    if (recentAttempts.length >= maxAttempts) {
        throw createError(
            ...
            ErrorTypes.RATE_LIMIT,
            "Bạn đã thử quá nhiều lần. Vui lòng chờ một chút.",
...
async function sendAutoVerifyNotification(member, role, guild) {
    try {
        ...
        const embed = createEmbed({
            title: "🎉 Chào mừng đến với máy chủ!",
            description: `Bạn đã được xác minh tự động tại **${guild.name}**!`,
            fields: [
                {
                    name: "✅ Vai trò đã được gán",
                    value: `Bạn hiện có vai trò ${role}!`,
                    inline: false
                },
                {
                    name: "📖 Tiếp theo là gì?",
                    value: "Giờ bạn có thể truy cập tất cả kênh và tính năng của máy chủ. Chào mừng bạn!",
                    inline: false
                }
            ],
            color: 'success'
...
export function validateAutoVerifyCriteria(criteria, accountAgeDays) {
    ...
    if (!validCriteria.includes(criteria)) {
        throw createError(
            ...
            "Vui lòng chọn một tiêu chí hợp lệ.",
    ...
    if (criteria === 'account_age') {
        if (!accountAgeDays || accountAgeDays < minAutoVerifyAccountAgeDays || accountAgeDays > maxAutoVerifyAccountAgeDays) {
            throw createError(
                ...
                ErrorTypes.VALIDATION,
                `Tuổi tài khoản phải nằm trong khoảng từ ${minAutoVerifyAccountAgeDays} đến ${maxAutoVerifyAccountAgeDays} ngày.`,
